...
const GLOBAL_EXTENSION_NAME = "Haidoku";
...
function setI18n(replaceList, doc=document){
	for(let i=0; i<replaceList.length; i++){
		let json = replaceList[i];
		let list = doc.querySelectorAll(json["selector"]);
...
function makeRemoveDomainList(domainList, domain){
	return domainList.filter((e)=>{return e!=domain});
}
